#!/usr/bin/env python3
"""Behavioral regressions for bounded loops and versioned specification approval."""
...
class RuntimeTests(unittest.TestCase):
    ...
    def test_compact_output_and_bounded_history_avoid_context_growth(self) -> None:
        ...
        self.run_cli(INTERVIEW, "start", "--idea", "a" * 2000)
        compact, short = self.run_cli(INTERVIEW, "status")
        full, long = self.run_cli(INTERVIEW, "status", "--full")
        self.assertLess(len(short.stdout), 1500)
        self.assertNotIn("state", compact)
    ...
    def test_nonstandard_json_numbers_are_rejected(self) -> None:
        for index, literal in enumerate(("NaN", "Infinity", "-Infinity")):
            with self.subTest(literal=literal):
                ...
                self.run_cli(RALPH, "start", "--id", run_id, "--goal", "strict JSON")
                path = self.state_path("ralph", run_id)
                raw = path.read_text(encoding="utf-8").rstrip()
                self.assertTrue(raw.endswith("}"))
                path.write_text(raw[:-1] + ',\n  "unvalidated_probe": ' + literal + "\n}\n", encoding="utf-8")
                error, cp = self.run_cli(RALPH, "status", "--id", run_id, "--full", expected=6)
