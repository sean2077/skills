...
| Convention guides | `skills/agent-scaffold/assets/conventions/` → `.agents/conventions/` | Generic daily guidance per selected domain, routed from the managed `AGENTS.md` block and byte-checked by verify; project docs hold project facts. `docs-reorganization.md` installs with `docs` and is reached from `docs.md`, not from its own route. |
