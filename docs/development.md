...
Models and reasoning effort are left to the host. Claude exposes Read/Grep/Glob, not editing, delegation, or shell tools, so the verifier inspects supplied evidence and returns checks it cannot run to the parent. Codex requests `read-only` and may inspect through its sandbox. Neither setting is a guarantee: a configured sandbox does not prove effective confinement, parent runtime overrides can change permissions, and a linked worktree or clone is not an isolation boundary because it shares the repository's Git configuration, objects, and refs. Inspect the actual host policy, and keep execution the parent's job or an explicitly authorized disposable environment.
