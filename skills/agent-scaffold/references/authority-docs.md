...
Keep project-authored guidance outside the markers. The inner worktree boundary belongs to the installer; `--profile light` removes that policy. The managed template uses one paragraph, list item, table row, or command per source line. Project-owned prose is not reflowed.
