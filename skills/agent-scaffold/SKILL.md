...
the dual-host hook wiring into `.claude/settings.json` and `.codex/hooks.json` (jq → python →
...
Windows specifics (enabling symlinks; `python` for hooks/subagents): `reference.md` §11.
...
pre-commit `--check` drift guard need **python** — no Node or `package.json`. Since the rest of the
harness already prefers python for hook JSON, subagents now install wherever the core does; without
python they are cleanly skipped, and the installer says how to enable them later.
