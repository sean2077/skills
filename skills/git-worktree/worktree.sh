#!/usr/bin/env bash
# worktree.sh — one-command worktree-per-change lifecycle.
#
...
# a fast-forward-only push and no history rewrites. The manual flow has several
# easy-to-miss steps (branch naming, .worktrees/ placement, sharing heavy dirs,
# merge-back + prune + push); this freezes them into one command.
#
# Subcommands:
...
# repo-relative *gitignored/untracked* dirs (e.g. "node_modules third_party") to
# hardlink-share into a new/release worktree — zero extra disk, and (when those
# dirs are ignored) kept out of `git status`. Submodule .git pointers inside the
# shared dirs are repointed at the shared common git dir automatically.
#
# Trunk defaults to $WORKTREE_TRUNK or "main"; override per-call with --trunk.
# Push is fast-forward-only; on rejection it tells you to ff-merge and retry. It
# never force-pushes — that stays the user's call.
set -euo pipefail
...
ensure_worktrees_ignored() {
    ...
    # writing the tracked .gitignore here would leave the trunk worktree dirty and
    # block a later `done`. Use the repo-local, untracked exclude file instead. For
    # a durable/shared rule, add `.worktrees/` to the committed .gitignore yourself
    # (see SKILL.md) — this is just the safety net.
...
# Repoint any submodule .git file inside a shared dir at the shared common gitdir,
# so the hardlinked pointers resolve from the new worktree.
repoint_submodules() {
    local wt="$1" sub
...
# Hardlink-share configured heavy dirs (intended for gitignored/untracked ones).
share_dirs() {
    local wt="$1" d
...
# Directory of the worktree that currently has <trunk> checked out (merge target).
...
cmd_new() {
    ...
    log "when done, run inside it: worktree.sh done   # merge back to $TRUNK + clean up + push"
}
...
$(git -C "$WT" status --short | head -10)"
    ...
    if git merge-base --is-ancestor "$BRANCH" "$TRUNK"; then
        ...
        git merge --no-ff "$BRANCH" -m "${MSG:-Merge $BRANCH}"
        log "merged → $TRUNK @ $(git rev-parse --short HEAD)"
    fi
    # Clean check passed above, so --force (needed when shared dirs contain submodules) is safe.
